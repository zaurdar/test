...
mk_cmd ='ffmpeg -i CVM3D/test.mp4 -vn -acodec copy CVM3D/videos/audio.aac ' \
    ...
    'ffmpeg -i CVM3D/videos/output_15.mp4 -i ' \
        'CVM3D/videos/audio.aac -c:v ' \
        'copy -c:a aac -map 0:v:0 -map 1:a:0 -shortest CVM3D/output.mp4 -y'
delete_cmd ="del CVM3D/videos/projo_1.mp4," \
...
st.title("citylight project video générator")
...
file = st.file_uploader("please put your image")
if file is not None:
    ...
    n_file_path = os.path.join(path,str(temp_dir)+'/fond.jpg')
...
with col1:
    ...
    color = st.color_picker("Sélectionnez une couleur", "#ff6347")

if st.button("enter"):
    test_fond_cmd = 'ffmpeg -i '+str(temp_dir)+'/fond.jpg'+' -vf ' \
                    'scale=1920:1080 '+str(temp_dir)+'/fond.jpg -y,' \
                    'ffmpeg -i '+str(temp_dir)+'/fond.jpg -vf "drawbox=x=' \
                    + str(x) + ':y=' + str(
        y) + ':w=120:h=210:color='+str(color)+'@0.5" '+str(temp_dir)\
                    +'/fond_1.jpg -y,' \
             'ffmpeg -i '+str(temp_dir)+'/fond_1.jpg -vf "drawbox=x=' \
                    + str(x) + ':y=' + str(
        y + 330) + ':w=120:h=210:color='+str(color)+'@0.5" '+str(temp_dir)\
                    +'/fond_2.jpg -y,' \
                   'ffmpeg -i '+str(temp_dir)+'/fond_2.jpg -vf "drawbox=x=' \
                    + str(x + 160) + ':y=' + str(
        y) + ':w=120:h=210:color='+str(color)+'@0.5" '+str(temp_dir)\
                    +'/fond_1.jpg -y,' \
             'ffmpeg -i '+str(temp_dir)+'/fond_1.jpg -vf "drawbox=x=' \
    ...
    with col2:
        ...
        ima = open(str(temp_dir) + '/fond_2.jpg', "rb").read()
        st.download_button("download video", ima, "image.jpg")
st.header('video creation')
file = st.file_uploader("please put your video")
if file is not None:
    # Créer un fichier temporaire
    temp_file = tempfile.NamedTemporaryFile(delete=False)
...
if st.button("create edit"):
    extract_cmd = 'ffmpeg -i '+file_path+' -vf scale=1920:1080 CVM3D/test.mp4 -y,' \
                ...
                'ffmpeg -i CVM3D/test.mp4 -vf crop=120:210:' + str(x + 6 * (160)) + ':' + str(y + 330) + ' CVM3D/videos/projo_14.mp4 -y,' \
                   ...
                   'scale=36:50 CVM3D/videos/temp_2.mp4 -y,' \
                   'ffmpeg -i CVM3D/videos/projo_3.mp4 -vf ' \
                   'scale=36:50 CVM3D/videos/temp_3.mp4 -y,' \
                   'ffmpeg -i CVM3D/videos/projo_4.mp4 -vf ' \
                   'scale=36:50 CVM3D/videos/temp_4.mp4 -y,' \
                   'ffmpeg -i CVM3D/videos/projo_5.mp4 -vf ' \
                   'scale=36:50 CVM3D/videos/temp_5.mp4 -y,' \
                   'ffmpeg -i CVM3D/videos/projo_6.mp4 -vf ' \
                   'scale=36:50 CVM3D/videos/temp_6.mp4 -y,' \
                   'ffmpeg -i CVM3D/videos/projo_7.mp4 -vf ' \
                   'scale=36:50 CVM3D/videos/temp_7.mp4 -y,' \
                   'ffmpeg -i CVM3D/videos/projo_8.mp4 -vf ' \
                   'scale=36:50 CVM3D/videos/temp_8.mp4 -y,' \
                   'ffmpeg -i CVM3D/videos/projo_9.mp4 -vf ' \
                   'scale=36:50 CVM3D/videos/temp_9.mp4 -y,' \
                   'ffmpeg -i CVM3D/videos/projo_10.mp4 -vf ' \
                   'scale=36:50 CVM3D/videos/temp_10.mp4 -y,' \
                   'ffmpeg -i CVM3D/videos/projo_11.mp4 -vf ' \
                   'scale=36:50 CVM3D/videos/temp_11.mp4 -y,' \
                   'ffmpeg -i CVM3D/videos/projo_12.mp4 -vf ' \
                   'scale=36:50 CVM3D/videos/temp_12.mp4 -y,' \
                   'ffmpeg -i CVM3D/videos/projo_13.mp4 -vf ' \
                   'scale=36:50 CVM3D/videos/temp_13.mp4 -y,' \
                   'ffmpeg -i CVM3D/videos/projo_14.mp4 -vf ' \
                   'scale=36:50 CVM3D/videos/temp_14.mp4 -y'
    execute(extract_cmd)
    st.write("ok")
    execute(mk_cmd)
